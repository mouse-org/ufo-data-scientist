const compare = require('../helpers/compareObjects')
...
test('January to be less than April', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'month', {name: 'January'}, {name: 'April'})
  ).toBe(-1)
})

test('January to be less than March', () => {
  expect(compare('name', 'unknown', 'datetime', 'month', {name: 'January'}, {name: 'March'})).toBe(-1)
})

test('April to be less than October', () => {
  expect(compare('name', 'unknown', 'datetime', 'month', {name: 'April'}, {name: 'October'})).toBe(-1)
})

test('October to be greater than April', () => {
  expect(compare('name', 'unknown', 'datetime', 'month', {name: 'October'}, {name: 'April'})).toBe(1)
})

test('October to be less than December', () => {
  expect(compare('name', 'unknown', 'datetime', 'month', {name: 'October'}, {name: 'December'})).toBe(-1)
})

test('December to be greater than October', () => {
  expect(compare('name', 'unknown', 'datetime', 'month', {name: 'December'}, {name: 'October'})).toBe(1)
})

test('"December 2019" to be greater than "October 2019"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'monthAndYear', {name: 'December 2019'}, {name: 'October 2019'})
  ).toBe(1)
})

test('"December 2018" to be less than "October 2019"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'monthAndYear', {name: 'December 2018'}, {name: 'October 2019'})
  ).toBe(-1)
})

test('"April 2018" to be less than "October 2019"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'monthAndYear', {name: 'April 2018'}, {name: 'October 2019'})
  ).toBe(-1)
})

test('"April 2018" to be less than "October 2018"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'monthAndYear', {name: 'April 2018'}, {name: 'October 2018'})
  ).toBe(-1)
})

test('"April 2018" to be greater than "March 2018"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'monthAndYear', {name: 'April 2018'}, {name: 'March 2018'})
  ).toBe(1)
})

test('"March 2018" to be less than "May 2018"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'monthAndYear', {name: 'March 2018'}, {name: 'May 2018'})
  ).toBe(-1)
})

test('"June 2018" to be greater than "May 2018"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'monthAndYear', {name: 'June 2018'}, {name: 'May 2018'})
  ).toBe(1)
})

test('"June 2018" to be less than "August 2018"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'monthAndYear', {name: 'June 2018'}, {name: 'August 2018'})
  ).toBe(-1)
})

test('"2" to be less than "10"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'date', {name: '2'}, {name: '10'})
  ).toBe(-1)
})

test('"02" to be less than "10"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'minute', {name: '02'}, {name: '10'})
  ).toBe(-1)
})

test('"Tuesday" to be less than "Friday"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'dayOfTheWeek', {name: 'Tuesday'}, {name: 'Friday'})
  ).toBe(-1)
})

test('"Sunday" to be less than "Thursday"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'dayOfTheWeek', {name: 'Sunday'}, {name: 'Thursday'})
  ).toBe(-1)
})

test('"Friday" to be greater than "Monday"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'dayOfTheWeek', {name: 'Friday'}, {name: 'Monday'})
  ).toBe(1)
})

test('"3 am" to be less than "1 pm"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'hour', {name: '3 am'}, {name: '1 pm'})
  ).toBe(-1)
})

test('"1 pm" to be greater than "5 am"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'hour', {name: '1 pm'}, {name: '5 am'})
  ).toBe(1)
})

test('"12 am" to be less than "5 am"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'hour', {name: '12 am'}, {name: '5 am'})
  ).toBe(-1)
})

test('"2 am" to be less than "12 pm"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'hour', {name: '2 am'}, {name: '12 pm'})
  ).toBe(-1)
})

test('"12 am" to be less than "11 am"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'hour', {name: '12 am'}, {name: '11 am'})
  ).toBe(-1)
})

test('"12 pm" to be less than "1 pm"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'hour', {name: '12 pm'}, {name: '1 pm'})
  ).toBe(-1)
})

test('"12 pm" to be greater than "11 am"', () => {
  expect(
    compare('name', 'unknown', 'datetime', 'hour', {name: '12 pm'}, {name: '11 am'})
  ).toBe(1)
})
